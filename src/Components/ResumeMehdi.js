...
function ResumeMehdi() {
    const [founders, setFounders] = useState({});
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        axios.get('https://back-end.a8m.ir/api/founders/2')
            .then(response => {
            ...
            <div className="resume-container">
                <div className="resume-header">
                    <div className="resume-name">
                        <h1>{founders.full_name || 'mehdi'}</h1>
                    </div>
                    <div className="resume-logo">
                        <img src={mehdi} alt="Founder Logo" className="circular-founder-logo" />
                    </div>
                </div>
